...
export class Protocol extends ssri.Trait {
  ...
  async updateProtocol(
    ...
    // Ensure at least one input for protocol creation transactions.
    const argsBytes = ccc.bytesFrom(this.script.args);
    const isNewProtocol =
      argsBytes.length === 0 ||
      (argsBytes.length === 32 && argsBytes.every((byte) => byte === 0));
    if (txReq.inputs.length === 0 && isNewProtocol) {
    ...
    return {
      ...
      script_code_hashes: {
        ...
        ckb_boost_tipping_type_code_hash: config.script_code_hashes
          ...
          : ccc.hexFrom("0x" + "0".repeat(64)),
        ckb_boost_achievement_type_code_hash: config.script_code_hashes
          ?.ckb_boost_achievement_type_code_hash
          ? ccc.hexFrom(
              config.script_code_hashes.ckb_boost_achievement_type_code_hash
            )
          : ccc.hexFrom("0x" + "0".repeat(64)),
      ...
      achievement_type_hashes: (config.achievement_type_hashes || []).map(
