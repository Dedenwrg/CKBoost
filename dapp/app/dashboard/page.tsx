...
const shorten = (value: string | null | undefined, head = 8, tail = 4): string => {
...
const normalizeHex = (value: unknown): string | null => {
  ...
  try {
    if (typeof value === "object" && ArrayBuffer.isView(value as ArrayBufferView)) {
...
const toMillis = (value: number | bigint | string | undefined | null): number | null => {
...
const formatPoints = (value: number | bigint | string | undefined | null): string => {
...
const buildSubmissionEntries = (
  ...
  return submissions.flatMap((submission, index) => {
    ...
    const quest = campaignData?.quests?.find((item) => Number(item.quest_id) === questId)

    const acceptedIds = quest?.accepted_submission_user_type_ids || []
    const isApproved = userTypeId
      ? acceptedIds.some((candidate) => normalizeHex(candidate)?.toLowerCase() === userTypeId.toLowerCase())
      : false
    ...
    const submissionTimestamp = toMillis(Number(submission.submission_timestamp))
...
export default function Dashboard() {
  ...
  const displayName = useMemo(() => {
    const identityData = currentUserData?.verification_data?.identity_verification_data
    if (typeof identityData === "string" && identityData.length > 0 && identityData !== "0x") {
      try {
  ...
  const isVerified = useMemo(() => {
    ...
    const telegramId = Number(currentUserData.verification_data?.telegram_personal_chat_id || 0)
    const identityBytes = currentUserData.verification_data?.identity_verification_data || ""
    return telegramId > 0 || (typeof identityBytes === "string" && identityBytes.length > 0)
  }, [currentUserData])
  ...
  const campaignsParticipated = useMemo(() => {
    const unique = new Set(submissionEntries.map((entry) => entry.campaignTypeId.toLowerCase()))
    return unique.size
  }, [submissionEntries])
  ...
  const upcomingDeadlines = useMemo(() => {
    ...
    return pendingSubmissions
      .filter((entry) => entry.deadlineTimestamp && entry.deadlineTimestamp > now)
  ...
  useEffect(() => {
    ...
    const loadTokenBalances = async () => {
      ...
      try {
        ...
        const gatherAmountForScript = async (script: ccc.Script): Promise<bigint> => {
          let total = 0n
        ...
        const pointsCodeHash = scriptHashes?.ckb_boost_points_udt_type_code_hash
  ...
  const [questFilter, setQuestFilter] = useState<"pending" | "approved">("pending")

  const renderQuestList = (entries: SubmissionDisplayEntry[], status: "pending" | "approved") => {
    if (loading) {
      return (
        ...
        {entries.map((entry) => {
          ...
          if (deadlineDate) {
            const daysLeft = Math.max(
              0,
              Math.ceil((entry.deadlineTimestamp! - Date.now()) / (1000 * 60 * 60 * 24))
            )
            deadlineBadge = (
          ...
          return (
            <div key={entry.key} className="border rounded-lg p-4 space-y-4">
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                <div>
                  <h3 className="font-semibold text-base">{entry.questTitle}</h3>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold">{formatPoints(entry.points)} pts</span>
                </div>
              ...
              <div className="flex items-center justify-between">
                <Progress value={entry.status === "approved" ? 100 : 40} className="h-2 w-3/4" />
                <Link href={`/campaign/${entry.campaignTypeId}/quest/${entry.questId}`}>
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-lg text-muted-foreground">
                  Track your submissions and monitor quest approvals in real time.
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{campaignsParticipated}</div>
                <p className="text-xs text-muted-foreground">Unique campaigns contributed</p>
              </CardContent>
            </Card>
          </div>
            ...
            <div className="lg:col-span-2">
              <Card>
                <CardHeader className="flex flex-col gap-4">
                  ...
                  <Tabs value={questFilter} onValueChange={(value) => setQuestFilter(value as "pending" | "approved")}>
                    <TabsList>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Balances are calculated from on-chain UDT cells held by your wallet.
                  </div>
                </CardContent>
              </Card>
                  ...
                  {upcomingDeadlines.length === 0 ? (
                    ...
                    upcomingDeadlines.map((entry) => {
                      ...
                      return (
                        ...
                        >
                          <div>
                            <div className="font-medium text-sm">{entry.questTitle}</div>
                            <div className="text-xs text-muted-foreground">
                              Due {deadline}
                            </div>
                          </div>
                ...
                <CardContent className="space-y-3">
                  {recentActivity.length === 0 ? (
                    ...
                    recentActivity.map((entry) => (
                      <div key={`activity-${entry.key}`} className="flex items-start gap-3">
                        {getActivityIcon(entry.status)}
                        <div className="flex-1 min-w-0">
                          <div className="text-sm">{formatActivityText(entry)}</div>
