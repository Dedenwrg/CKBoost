...
const log = createScopedLogger("QuestDialog");
...
export function QuestDialog({
  ...
  const [udtRewards, setUdtRewards] = useState<
    Array<{ token?: UDTToken; amount: string }>
  >([]);
  ...
  const toUDTAsset = (
    token: UDTToken,
    amountStr: string
  ): { udt_script: ccc.ScriptLike; amount: bigint } | null => {
  ...
  const handleSubtaskChange = (
    index: number,
    field: keyof QuestSubTaskDataLike,
    value: unknown
  ) => {
    ...
    if (open && questForm.rewards_on_completion[0]) {
      ...
      const rewardsFromUdts = (base.udt_assets || []).map((asset) => {
        ...
        return {
          token,
          amount: token
            ? udtRegistry.formatAmount(Number(asset.amount), token)
            : asset.amount.toString(),
        };
      });

      // Map CKB amount (if any) as a pseudo token
      const rewards: Array<{ token?: UDTToken; amount: string }> = [
        ...rewardsFromUdts,
      ];
  ...
  const handleAddUDTReward = () => {
    log.info("Adding UDT reward, current count:", udtRewards.length);
  ...
  const handleFillTestQuest = () => {
    ...
    onQuestFormChange({
      ...
      metadata: {
        ...
        long_description:
          "Learn the basics of smart contract development on CKB by writing a simple contract that demonstrates basic functionality",
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Configure CKB and UDT tokens to distribute as quest rewards
                </p>
              </div>
              <Button
                type="button"
                  ...
                  <Card key={index} className="p-4">
                    <div className="space-y-3">
                      ...
                      <UDTSelector
                        value={reward}
                        onChange={(value) =>
                          handleUDTRewardChange(index, value)
                        }
                        signer={signer}
                        showBalance={true}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      onQuestFormChange({
                        ...questForm,
                        initial_quota: sanitizedValue,
                      });
                    }}
                    placeholder="Enter initial quota (minimum 1, default 10)"
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                    The initial quota of reward completions (minimum 1, default
                    10). Can be further funded.
                  </p>
                </div>
                ...
                {setLockUDTOnSave && (
                  <div className="flex items-center space-x-2 p-3 bg-muted rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Automatically lock the required UDT tokens to this
          ...
          <div className="space-y-4">
            ...
            {questForm.sub_tasks.length === 0 ? (
              <div className="text-center py-4 border rounded-lg text-sm text-muted-foreground">
                No subtasks added yet. Add subtasks to break down the quest into
                steps.
              </div>
            ) : (
              <div className="space-y-3">
                {questForm.sub_tasks.map(
                  (subtask: QuestSubTaskDataLike, index: number) => (
                    <Card key={index} className="p-4">
                      <div className="space-y-3">
