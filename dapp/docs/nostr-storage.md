...
### 13. Author-Based Indexing

Some product flows (like tipping proposal comments) benefit from querying "all
events written by a deterministic author" instead of storing individual event
IDs. Nostr already supports this via the `authors` field on `NostrFilter`. Our
implementation now ships a small toolkit that makes it easy to adopt this
pattern:

1. **Derive deterministic key pairs** –
   `lib/nostr/author-index.ts` exports helpers such as `deriveAuthorKeys` and
   `buildAuthorFilter`. You can derive a key pair from any stable seed. For
   example, a CKB type ID can be reused as a private key:

   ```ts
   import { deriveAuthorKeys } from "@/lib/nostr/author-index";

   const tippingAuthor = deriveAuthorKeys({ privateKey: campaignTypeId });
   console.log(tippingAuthor.pubkey); // use in UI / filters
   ```

2. **Publish events with deterministic authors** – every mutation in
   `useNostrStorage` (`storeSubmission`, `storeCampaignContent`,
   `storeAchievementMetadata`) now accepts an optional `authorIndex`. For fully
   custom payloads we added `storeAuthorIndexedEvent`, which takes arbitrary
   `tags`, `metadata`, `kind`, and the author index configuration:

   ```ts
   const { storeAuthorIndexedEvent } = useNostrStorage();

   const { neventId, author } = await storeAuthorIndexedEvent.mutateAsync({
     content: commentHtml,
     authorIndex: { privateKey: campaignTypeId },
     tags: [
       ["type", "tipping_comment"],
       ["campaign", campaignTypeId],
     ],
   });
   console.log(neventId, author);
   ```

3. **Query via `authors` filters** – `useNostrFetch` exposes
   `fetchAuthorIndexedEvents` which accepts either a hex/`npub` string or an
   `AuthorIndexConfig`. It automatically builds a `NostrFilter` populated with
   the derived pubkey so you can enumerate every event written with that key:

   ```ts
   const { fetchAuthorIndexedEvents } = useNostrFetch();

   const events = await fetchAuthorIndexedEvents(
     { privateKey: campaignTypeId },
     {
       filter: {
         kinds: [30078],
         limit: 100,
   ...
   If you already know the pubkey, pass the hex or `npub1…` string directly.

These helpers allow us to treat "author" as a deterministic index – perfect for
grouping comments, metadata, or any content family under a shared keypair that
can be recomputed from on-chain identifiers.
