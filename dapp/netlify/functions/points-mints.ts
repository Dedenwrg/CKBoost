...
export const handler: Handler = async (event) => {
  ...
  const protocolTypeHashType = (
    process.env.NEXT_PUBLIC_PROTOCOL_TYPE_HASH_TYPE?.trim() || "type"
  ) as ccc.HashType;
    ...
    try {
      ...
      for (const cell of inputCells) {
        ...
        try {
          ...
          console.warn(
            `[points-mints] Failed to load input cell for tx ${match.txHash} index ${index}:`,
            error
          );
        }
      }
      ...
      console.warn(
        `[points-mints] Failed to process transaction ${match.txHash}`,
        error
      );
    }
  }
