...
use ckboost_shared::{
    types::{
        Byte32 as SharedByte32, ConnectedTypeID, Uint128, Uint64, UserData,
        UserSubmissionRecordVec, UserVerificationData,
    },
    Error,
};
...
impl CKBoostUser for CKBoostUserType {
    ...
    fn submit_quest(tx: Option<Transaction>, user_data: UserData) -> Result<Transaction, Error> {
        ...
        match connected_type_id {
            Ok(connected_type_id) => {
                ...
                let current_user_cell =
                    find_cell_by_out_point(user_outpoint).map_err(|_| Error::UserCellNotFound)?;
    ...
    fn verify_submit_quest(context: &TransactionContext<RuleBasedClassifier>) -> Result<(), Error> {
